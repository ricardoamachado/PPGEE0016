...
com um algortimo de \emph{line search} exato para cada iteração do
\emph{steepest descent} é elevado. Consequentemente, utiliza-se o algortimo de
\emph{backtracking} para obter um tamanho do passo $\alpha_k$ que gere um
decrescimento suficiente em cada iteração.

As condições de Goldstein descritas em CITAR LIVRO definem critérios para que o
tamanho do passo $\alpha$ resulte num decrescimento suficiente sem ser pequeno
demais.

Com base nas condições de Goldstein, define-se o algoritmo de
\emph{backtracking line search} cujo passo a passo é listado abaixo.
